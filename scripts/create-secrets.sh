...
# Create annotations-db-credentials secret if it does not exist
if ! kubectl get secret annotations-db-credentials &>/dev/null ; then
    ...
    echo "Secret annotations-db-credentials already found. Skipping creation..."
fi
